(function() {
  ...
  function updateChart(json, scope, element) {
    ...
    svg.selectAll('.h-grid').data(y.ticks(yTicks)).enter().append('line').attr(
        {
          x1: 0,
          y1: y,
          x2: width,
    ...
    svg.selectAll('.v-grid').data(x.ticks(xTicks)).enter().append('line').attr(
        {
          x1: x,
          y1: 0,
          x2: x,
    ...
    var legItem = svg.append('g').attr('transform',
        ...
        '.legend-item').data(scope.series).enter().append('g').attr('class',
        'legend-item');
